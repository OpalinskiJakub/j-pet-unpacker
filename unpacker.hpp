      ...
      { // TODO: finish :) and validate
        ...
        foreach_preprocessed_data:
          for (uint32_t ei = 0; qi < raw_data.size() && ei < endp_len; ei++, qi++)
          {
            ...
            if (ei != 0 && ei != endp_len - 1)
            { // process only data between the header and the trailer

              if (ei % 2 == 1)
              { // odd is the lower data part
...
int32_t unpacker::get_time_window_repaired(meta_t& fixed_meta_data, std::unordered_map<uint32_t, std::vector<hit_t>>& fixed_data, std::istream& fp)
{
  ...
  if (ret == -1 || ret == 0)
  { // return normally on error or eof. There's nothing to be fixed.
    return ret;
  }
  ...
  if (ret == 2)
  {
    std::unordered_map<uint32_t, uint32_t>
        late_endpoints; // those endpoints arrived late. The tw of the rest of endpoints must be taken from previous tw
    ...
    for (auto const& pair : meta_data.endp_stats)
    {
      if (pair.second.trigger_id != ((meta_data.tw_trigger_id) & 0xFF))
      {                                                      // endpoint tw is a mismatch (endpoints was late)
        ...
        if (original_data.count(pair.first))
        { // make sure there's anything to be assigned from
    ...
    for (auto const& pair : prev_meta_data.endp_stats)
    { // take data from previous tw
      if (!late_endpoints.count(pair.first))
      { // must not be in 'late_endpoints' list
        // save the data of this endpoint
        fixed_endpoints[pair.first] = pair.second.trigger_id;
        fixed_meta_data.endp_stats[pair.first] = pair.second;

        if (prev_original_data.count(pair.first))
        { // make sure there's anything to be assigned from
    ...
    if (fixed_endpoints.size() != 0)
    {
      ...
      {
        if (buff != pair.second)
        { // at least one trigger_id in 'fixed_endpoints' list differ...
          if (verbosity > 1)
          {
...
void unpacker::calculate_time(uint32_t endp_id, std::vector<hit_t>& v,
                              std::unordered_map<uint32_t, std::unordered_map<uint32_t, std::vector<uint32_t>>>& tdc_calib)
{
  ...
  while (i != std::end(v))
  {
    ...
    else
    {
      ...
      if (kCapTimeAtThreshold && i->channel_id != 104)
      { // don't delete reference sample
